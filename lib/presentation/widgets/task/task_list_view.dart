...
class TaskListView extends StatelessWidget {
    ...
    return BlocBuilder<TaskBloc, TaskState>(
      builder: (context, state) {
        ...
        if (state is TaskLoaded) {
          // 🚀 MEJORA 1: Filtrar el estado general para trabajar en el Home ÚNICAMENTE con pendientes
          final tareasPendientes = state.tareas.where((t) => !t.completada).toList();
          ...
          // 🚀 MEJORA 2: Aplicar los filtros de pestañas ('Todas', 'Hoy', 'Próximas') sobre las pendientes
          ...
          if (activeFilter == 'Hoy') {
            filteredTasks = tareasPendientes.where((task) {
              ...
              final tStr = "${task.fechaVencimiento!.year}-${task.fechaVencimiento!.month.toString().padLeft(2, '0')}-${task.fechaVencimiento!.day.toString().padLeft(2, '0')}";
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              String labelPrioridad = tarea.prioridad == 'A' ? 'Alta' : (tarea.prioridad == 'B' ? 'Baja' : 'Media');
              Color colorPrioridad = tarea.prioridad == 'A' ? Colors.red : (tarea.prioridad == 'B' ? Colors.blue : Colors.orange);
              ...
              if (tarea.categoriaInfo != null) {
                ...
                catColor = _parseHexColor(tarea.categoriaInfo!['color_hex'] ?? '#4E9F3D');
              }
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    // 🚀 MEJORA 3: Checkbox completamente conectado a tu flujo reactivo BLoC
                    Checkbox(
                          ...
                          Row(
                            children: [
                              ...
                              Container(
                                ...
                                child: Text(
                                  labelPrioridad,
                                  style: TextStyle(color: colorPrioridad, fontSize: 11, fontWeight: FontWeight.bold),
                                ),
                              ),
                            ],
                          ),
                    ...
                    PopupMenuButton<String>(
                      onSelected: (val) {
                        if (val == 'edit') {
                          ...
                          context.read<TaskBloc>().add(TaskDeleteEvent(tarea.id!));
                        }
                      },
  ...
  void _showEditBottomSheet(BuildContext context, TaskModel tarea) {
    ...
    String prioridadSeleccionada = tarea.prioridad == 'A' ? 'Alta' : (tarea.prioridad == 'B' ? 'Baja' : 'Media');
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: ['Baja', 'Media', 'Alta'].map((p) {
                      ...
                      return GestureDetector(
                        onTap: () => setModalState(() => prioridadSeleccionada = p),
                        child: Container(
                          margin: const EdgeInsets.only(right: 8),
                          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                          decoration: BoxDecoration(
                            color: isSel ? AppColors.primaryGreen : const Color(0xFFF5F5F5),
                            borderRadius: BorderRadius.circular(12),
                          ),
                          child: Text(
                            p,
                            style: TextStyle(color: isSel ? Colors.white : AppColors.textMuted, fontWeight: FontWeight.bold),
                          ),
                        ),
                      );
                    }).toList(),
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    hint: const Text('Selecciona una categoría', style: TextStyle(color: AppColors.textMuted)),
                    items: categorias.map((cat) {
                      return DropdownMenuItem<int>(
                        value: cat.id,
                        child: Text(cat.nombre, style: const TextStyle(color: AppColors.textDark)),
                      );
                    }).toList(),
                    onChanged: (val) => setModalState(() => categoriaSeleccionadaId = val),
                  ),
